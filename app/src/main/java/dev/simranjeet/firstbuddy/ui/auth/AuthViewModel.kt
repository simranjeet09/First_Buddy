...
class AuthViewModel(private val repository: UserRepository
    ...
    fun forgetPassword(){
        ...
        //calling forgot from repository to perform the  authentication
        val disposable = repository.forgot(email!!)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({
                //sending a success callback
                authListener?.onFailure("Email with password reset instructions is sent to registered email. ");
            }, {
                //sending a failure callback
                authListener?.onFailure(it.message!!)
            })
